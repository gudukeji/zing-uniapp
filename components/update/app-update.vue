	...
	export default {
		...
		methods: {
			...
			getUpdateInfo() {
				...
				const platformType = this.platform === "ios" ? 2 : 1;
				console.log("请求平台类型:", platformType);

				//向后台发起请求，获取最新版本号
				getUpdateInfo(platformType)
					.then((res) => {
						...
						console.log("缓存日期:", tagDate);
						console.log("当前日期:", nowDate);
						console.log("getVer参数:", this.getVer);
						console.log("is_force:", res.data.is_force);

						// 如果是手动检查更新（getVer: true），直接跳过缓存逻辑
						if (this.getVer) {
							console.log("手动检查更新，跳过缓存逻辑，直接检查版本");
						} else {
							// 自动检查更新的缓存逻辑
							if (tagDate !== nowDate) {
								console.log("设置缓存日期并继续");
								uni.setStorageSync('app_update_time', new Date().toLocaleDateString());
							} else if (tagDate == nowDate && !res.data.is_force) {
								console.log("今日已检查且非强制更新，跳过");
								return;
							}
						}
						...
						if (!vm.update_info.platform) {
							console.log("后台未配置当前系统的升级数据");
						} else {
							console.log("开始检查是否需要更新");
							vm.checkUpdate(); ///检查是否更新
						}
					})
					.catch((err) => {
						console.error("获取更新信息失败:", err);
						vm.popup_show = false;
						uni.showToast({
							title: '检查更新失败',
							icon: 'none'
			...
			checkUpdate() {
				console.log("检查版本更新...");
				console.log("当前版本:", vm.version);
				console.log("线上版本:", vm.update_info.version);

				vm.need_update = vm.compareVersion(vm.version, vm.update_info.version); // 检查是否需要升级
				console.log("是否需要更新:", vm.need_update);

				if (vm.need_update) {
					console.log("需要更新，显示升级弹窗");
					...
					console.log("当前已是最新版本，触发isNew事件");
					this.$emit('isNew')
				}
			},
			...
			compareVersion(ov, nv) {
				console.log("版本比较 - 当前版本:", ov, "线上版本:", nv);

				if (!ov || !nv || ov == "" || nv == "") {
					console.log("版本号为空，返回false");
					return false;
				}
				let b = false,
					ova = ov.split(".", 4),
				...
				console.log("当前版本数组:", ova);
				console.log("线上版本数组:", nva);
				...
				if (nva.length > ova.length && 0 == nv.indexOf(ov)) {
					console.log("线上版本更长且包含当前版本，需要更新");
					return true;
				} else {
					console.log("版本比较完成，不需要更新");
					return false;
				}
			},
		},
